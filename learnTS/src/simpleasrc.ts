(function (window, $, _) {
  ...
  function selectItem($selectItem, $multiSelectForm) {
    ...
    if (formData.activeLimit === 1 && $selectListItem) {
      _.forEach($selectListItem.siblings(), function (listItem: object) {
  ...
  window.SearchPartialUpdated = function () {
  ...
  window.AddMultiSelectAction = function ($form) {
    var data = $form.data();
  ...
  $.fn.Search = function (options: any) {
    ...
    function clearSearch(e) {
      ...
      _.forEach(settings.DropdownIds, function (id) {
        $targetElement.find(id).find('option').first().prop('selected', true);
      });
      search(settings, setFocus);
    }
    ...
    // function to search
    function search(searchSetting, callback) {
  ...
  $.fn.lsCategoryFilter = function (params) {
    ...
    $linkSelector.on('click', function (e: any) {
      e.preventDefault();
    ...
    $filterSelector.on('click', function (e: any) {
      e.preventDefault();
      e.stopPropagation();
      e.stopImmediatePropagation();
